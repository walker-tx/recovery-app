...
async function fixture(t) {
  const root = await fs.realpath(
    await fs.mkdtemp(path.join(os.tmpdir(), "isolated dispatch-")),
  );
  ...
  const run = (file, args, extra = {}) =>
    spawnSync(path.join(root, "scripts", file), args, {
      cwd: root,
      env,
      encoding: "utf8",
      timeout: 5000,
...
test("existing local entrypoints forward isolated arguments exactly before any legacy work", async (t) => {
  ...
  for (const [file, command] of wrappers.filter(
    ([, targetCommand]) => targetCommand,
  )) {
    const argument =
      command === "start"
        ? "/tmp/Convex binary 'quoted' $literal;never-execute"
        : "6c64e416-fd56-4afd-917b-bcebc51d169f";
    const result = f.run(file, ["--isolated", argument]);
    assert.equal(result.status, 0, result.stderr);
    assert.deepEqual(JSON.parse(result.stdout), [
      path.join(f.root, "scripts/stack-runtime.cjs"),
      command,
      argument,
    ]);
    assert.equal(
      f.run(file, ["--isolated", argument], {
        env: { ...f.env, FIXTURE_EXIT: "19" },
      }).status,
      19,
    );
  }
...
test("isolated remote mode is refused without falling through to legacy routes", async (t) => {
  const f = await fixture(t),
    result = f.run("zero-tailnet.sh", ["--isolated"]);
...
test("isolated status requires the selected checkout root, without changing legacy status", async (t) => {
  const f = await fixture(t),
    result = f.run(
      "status.sh",
      ["--isolated", "6c64e416-fd56-4afd-917b-bcebc51d169f"],
      { cwd: path.dirname(f.root) },
    );
  assert.equal(result.status, 1);
  assert.match(result.stderr, /repository root/);
  assert.equal(
    f.run("status.sh", [], { cwd: path.dirname(f.root) }).status,
    23,
  );
});
test("existing runtime validation rejects unsupported isolated arguments before opening anything", async () => {
  ...
  for (const args of [
    ["start"],
    ["start", "--tailnet"],
    ["start", "/synthetic", "--tailnet"],
    ["status"],
    ["stop", "wrong"],
    ["stop", "6c64e416-fd56-4afd-917b-bcebc51d169f", "extra"],
  ]) {
...
test("installed Mise forwards isolated flags through the existing task declarations", async (t) => {
  const f = await fixture(t),
    config = await fs.readFile(path.join(__dirname, "../mise.toml"), "utf8");
  ...
  const env = {
    ...f.env,
    XDG_CONFIG_HOME: f.root,
    XDG_DATA_HOME: f.root,
    XDG_CACHE_HOME: f.root,
    MISE_TRUSTED_CONFIG_PATHS: f.root,
    MISE_AUTO_INSTALL: "false",
    MISE_NO_ENV: "1",
    MISE_NO_HOOKS: "1",
  };
  for (const [task, command, arg] of [
    ["zero", "start", "/tmp/synthetic backend"],
    ["status", "status", "6c64e416-fd56-4afd-917b-bcebc51d169f"],
    ["stop", "stop", "6c64e416-fd56-4afd-917b-bcebc51d169f"],
  ]) {
    const result = spawnSync(
      mise,
      ["run", "--quiet", task, "--", "--isolated", arg],
  ...
  const rejected = spawnSync(
    mise,
    ["run", "--quiet", "zero:tailnet", "--", "--isolated"],
    { cwd: f.root, env, encoding: "utf8", timeout: 15000 },
  );
