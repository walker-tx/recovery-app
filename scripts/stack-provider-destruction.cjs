...
async function inspect(file, directory = false) {
  ...
  if (
    ...
    (st.mode & 0o077) !== 0 ||
    (directory ? !st.isDirectory() : !st.isFile() || st.nlink !== 1)
  ) {
    throw Error("Unsafe provider state");
  }
...
async function destroyProvider({ worktree, confirmation, readOwned, status }) {
  ...
  if (
    !confirmation ||
    confirmation.operation !== "destroy-provider-identity" ||
    !Object.entries(target).every(
      ([key, value]) => confirmation[key] === value,
    ) ||
    !Array.isArray(confirmation.affectedDomains) ||
    confirmation.affectedDomains.length !== 2 ||
    !["provider-data", "provider-signing-identity"].every((domain) =>
      confirmation.affectedDomains.includes(domain),
    )
  ) {
    throw Error("Provider destruction confirmation mismatch");
  }
  const verify = async () => {
    if (!isDeepStrictEqual(await readOwned(record.stackId), record)) {
      throw Error("Provider ownership changed");
    }
    const current = await status();
    if (
      !Object.entries(target).every(([key, value]) => current[key] === value) ||
      !isDeepStrictEqual(current.ports, record.ports) ||
      current.state === "conflict" ||
      current.services.provider !== "stopped"
    ) {
      throw Error("Provider must remain stopped and owned");
    }
  };
  await verify();
  ...
  for (const dir of [root, provider]) {
    ...
    const file = path.join(dir, markerName),
      st = await inspect(file);
    if (st.size > 4096 || (await fs.readFile(file, "utf8")) !== marker) {
      throw Error("Provider marker mismatch");
    }
    owned.set(file, st);
  }
  const entries = await fs.readdir(provider);
  if (
    !entries.includes("state.sqlite") ||
    entries.some((file) => ![...files, markerName].includes(file))
  ) {
  ...
  for (const file of selected) {
    owned.set(
      path.join(provider, file),
      await inspect(path.join(provider, file)),
    );
  }
  const checkFiles = async () => {
    const expected = [
      markerName,
      ...selected.filter((file) => owned.has(path.join(provider, file))),
    ].sort();
    if (!isDeepStrictEqual((await fs.readdir(provider)).sort(), expected)) {
      throw Error("Provider inventory changed");
    }
    for (const [file, original] of owned) {
      ...
      if (
        observed.dev !== original.dev ||
        observed.ino !== original.ino ||
        observed.mode !== original.mode ||
        (file !== root && file !== provider && observed.size !== original.size)
      ) {
        throw Error("Provider storage identity changed");
      }
    }
    for (const dir of [root, provider]) {
      if ((await fs.readFile(path.join(dir, markerName), "utf8")) !== marker) {
        throw Error("Provider marker changed");
      }
    }
  };
  await checkFiles();
  ...
  const report = {
    ...
    trustRepairRequired: true,
    lifecycleLockRetained: true,
    retirement: "uncertain",
    storage: Object.fromEntries(
      selected.map((file) => [file, "not-attempted"]),
    ),
  };
  ...
  try {
    ...
    await intent.writeFile(
      JSON.stringify({ version: 1, operation: report.operation, ...target }),
    );
    await intent.sync();
    await intent.close();
    intent = undefined;
    ...
    owned.set(
      path.join(root, retirementName),
      await inspect(path.join(root, retirementName)),
    );
