...
async function fixture(t, options = {}) {
  ...
  const worktree = path.join(root, 'worktree'), sibling = path.join(root, 'sibling');
  await Promise.all([fs.mkdir(worktree), fs.mkdir(sibling)]);
  const registryPath = path.join(root, 'registry');
  return { registry: createRegistry({ registryPath, portAvailable: async () => true, ...options }), registryPath, worktree, sibling };
}
test('concurrent siblings and repeated preparation retain independent identities/ports', async t => {
  ...
  const [a, b, again] = await Promise.all([registry.reserve(worktree), registry.reserve(sibling), registry.reserve(worktree)]);
  assert.deepEqual(a, again);
  assert.notEqual(a.stackId, b.stackId);
  assert.notEqual(a.providerGeneration, b.providerGeneration);
  assert.equal(new Set([...Object.values(a.ports), ...Object.values(b.ports)]).size, 12);
...
test('occupied ports skipped initially; existing reservations never silently move', async t => {
  ...
  const { registry, worktree } = await fixture(t, { portAvailable: async port => !busy.has(port) });
...
test('bounded lock contention never breaks a stale lock based on PID', async t => {
  const { registry, registryPath, worktree } = await fixture(t, { lockTimeoutMs: 30 });
  await fs.mkdir(registryPath, { mode: 0o700 });
  await fs.mkdir(path.join(registryPath, 'lock'));
  await fs.writeFile(path.join(registryPath, 'lock', 'owner.json'), '{"pid":99999999}');
...
test('release explicit, ownership checked, idempotent and sibling safe', async t => {
  const { registry, worktree, sibling } = await fixture(t);
  const a = await registry.reserve(worktree), b = await registry.reserve(sibling);
...
test('fake ownership permits resume; PID reuse blocks release and resume', async t => {
  const processes = new Map(), busy = new Set();
  const { registry, worktree } = await fixture(t, { portAvailable: async port => !busy.has(port), inspectProcess: async pid => processes.get(pid) ?? null });
  const a = await registry.reserve(worktree);
  const identity = { pid: 123, startedAt: 'boot-1:12345', worktree: a.worktree, stackId: a.stackId };
  processes.set(123, identity); busy.add(a.ports.metro);
  ...
  await assert.rejects(registry.release(worktree, a.stackId), /process/);
  processes.delete(123); busy.clear();
  await registry.release(worktree, a.stackId);
});
test('corrupt registry fails closed without overwrite', async t => {
  ...
  assert.equal(await fs.readFile(path.join(registryPath, 'registry.json'), 'utf8'), 'broken');
});
...
test('separate bootstrap processes serialize through the persistent registry', async t => {
  ...
  const results = await Promise.all([worktree, sibling].map(w => run(process.execPath, ['-e', program, require.resolve('./stack-registry.cjs'), registryPath, w], { timeout: 3000 })));
...
test('unsafe persisted JSON fails closed before every operation without overwrite', async t => {
  const { registry, registryPath, worktree: requestedWorktree, sibling: requestedSibling } = await fixture(t);
  const a = await registry.reserve(requestedWorktree);
  const b = await registry.reserve(requestedSibling);
  const worktree = a.worktree, sibling = b.worktree;
  const file = path.join(registryPath, 'registry.json');
  const original = JSON.parse(await fs.readFile(file, 'utf8'));
  const identity = { pid: 123, startedAt: 'boot:123', worktree: a.worktree, stackId: a.stackId };
  const mutations = [
    d => { d.stacks[sibling].ports.metro = 0; },
    d => { d.stacks[sibling] = null; },
    ...[0, -1, 65536, 24000.5].map(value => d => { d.stacks[sibling].ports.metro = value; }),
    d => { d.stacks[sibling].ports.metro = d.stacks[sibling].ports.provider; },
    d => { d.stacks[sibling].ports.metro = a.ports.metro; },
    d => { d.stacks[sibling].stackId = 'not-uuid'; },
    d => { d.stacks[sibling].stackId = a.stackId; },
    d => { d.stacks[sibling].providerGeneration = a.providerGeneration; },
    d => { d.stacks[sibling].owner = 'device:inode'; },
    d => { d.stacks[sibling].owner = '1:9007199254740992'; },
    d => { d.stacks.relative = { ...d.stacks[sibling], worktree: 'relative' }; delete d.stacks[sibling]; },
    ...[[], false, 'map', { unknown: identity }, { metro: null }].map(value => d => { d.stacks[worktree].processes = value; }),
    ...[{ ...identity, pid: 0 }, { ...identity, pid: 1.5 }, { ...identity, startedAt: {} }, { ...identity, startedAt: '' }, { ...identity, worktree: sibling }, { ...identity, stackId: 'wrong' }].map(value => d => { d.stacks[worktree].processes.metro = value; }),
  ];
  for (const [index, mutate] of mutations.entries()) {
    ...
    for (const operation of [
      () => registry.status(worktree), () => registry.reserve(worktree),
      () => registry.release(worktree, a.stackId),
      () => registry.recordProcess(worktree, a.stackId, 'metro', identity),
    ]) {
      await assert.rejects(operation, /Invalid (registry|reservation)/, `mutation ${index}`);
...
test('only explicit null inspection confirms recorded process absence on a free port', async t => {
  let actual;
  const { registry, worktree } = await fixture(t, { inspectProcess: async () => actual });
  const a = await registry.reserve(worktree);
  actual = { pid: 123, startedAt: 'boot:123', worktree: a.worktree, stackId: a.stackId };
