...
async function fixture(t, mode = "ready") {
  ...
  for (const [i, group] of groups.entries()) {
    ...
    daemons.set(
      `recovery-local/recovery-${record.stackId}-${group[0]}`,
      identity,
    );
  }
  if (mode === "split-pair") {
    ...
    processes.set(
      102,
      daemons.get(`recovery-local/recovery-${record.stackId}-convexCloud`),
    );
  }
  const probes = [],
    signals = [],
  ...
  const runtime = await createRuntime({
    ...
    identity: {
      ...
      identify: async (name) => {
        ...
        if (mode === "mapping-failed") {
          throw Error("secret mapping detail");
        }
        return mode === "replacement"
          ? { ...original, startedAt: "replacement" }
          : original;
      },
    },
    portAvailable: async () => true,
    ...
    fetchImpl: async (url, { signal }) => {
      ...
      if (mode === "failed") {
        throw Error("secret response detail");
      }
      return new Response(
        url.endsWith("/status")
          ? "packager-status:running"
          : url.endsWith("/instance_name")
            ? "local-name"
            : "{}",
      );
    },
    ...
    inherited: new Proxy(
      ...
      {
        get() {
          throw Error("Unexpected environment read");
        },
      },
    ),
  });
...
test("status probes verified original daemons once and labels site transport only", async (t) => {
  ...
  for (const service of Object.keys(f.record.ports)) {
...
for (const mode of ["replacement", "mapping-failed"]) {
  test(`status leaves ${mode} ownership unprobed`, async (t) => {
    ...
    assert.ok(
      Object.values(status.readiness).every(
...
test("status refuses both endpoints of a split daemon pair", async (t) => {
  ...
  assert.deepEqual(status.readiness.convexCloud, {
    state: "unknown",
    reason: "not-probed",
  });
  assert.deepEqual(status.readiness.convexSite, {
    state: "unknown",
    reason: "not-probed",
  });
});
...
test("status never probes stopped or mismatched original processes", async (t) => {
  ...
  for (const service of [
    ...
    assert.equal(status.readiness[service].state, "unknown");
  }
});
