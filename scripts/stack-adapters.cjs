...
function createPitchforkRunner({
  ...
  return async (
    ...
    const run =
      ...
      args.length > 7;
    if (!stop && !run) {
      throw Error("Unsupported Pitchfork command");
    }
    if (
      run &&
      [baseEnv, env].some(
...
async function inspectLinuxProcess(
  ...
  const check = () => {
    if (signal?.aborted) {
      throw Error("Process inspection aborted");
    }
  };
  ...
  try {
    ...
    if (!/^[a-zA-Z0-9-]+$/.test(boot)) {
      throw Error("Invalid boot identity");
    }
    check();
  ...
  throw Error("Cannot inspect process identity");
}
