...
async function preflightDestruction({
  worktree,
  registryPath,
  target,
  confirmation,
  ...
  try {
    if (
      ...
      !uuid(target.stackId) ||
      !uuid(target.providerGeneration) ||
      target.stackId === target.providerGeneration
    ) {
      throw Error();
    }
    const st = inspect(worktree, false, true);
    if (
      !absolute(registryPath) ||
      fs.realpathSync(registryPath) !== registryPath
    ) {
      throw Error();
    }
    inspect(registryPath, true, true);
    const file = path.join(registryPath, "registry.json");
    if (inspect(file, true, false).size > 1024 * 1024) {
      throw Error();
    }
    ...
    if (
      data.version !== 1 ||
      !record ||
      record.stackId !== target.stackId ||
      record.providerGeneration !== target.providerGeneration ||
      ...
      !names.every(
        (n) =>
          Number.isSafeInteger(record.ports[n]) &&
          record.ports[n] > 0 &&
          record.ports[n] <= 65535,
      ) ||
      new Set(Object.values(record.ports)).size !== names.length ||
      Object.keys(record.processes).some((n) => !names.includes(n))
    ) {
      throw Error();
    }
  } catch {
    block("target-mismatch", "reservation");
    return result;
  }
  ...
  result.affectedDomains = [
    ...
    {
      domain: "inbox",
      paths: ["mailpit.sqlite", "mailpit.sqlite-wal", "mailpit.sqlite-shm"].map(
        (n) => path.join(root, n),
      ),
    },
  ...
  result.confirmationAccepted = Boolean(
    confirmation &&
    ...
    new Set(confirmation.affectedDomains).size ===
      expected.affectedDomains.length &&
    confirmation.affectedDomains.every((domain) =>
      expected.affectedDomains.includes(domain),
    ),
  );
  ...
  for (const [file, code] of [
    [path.join(worktree, ".recovery-stack-lifecycle.lock"), "lifecycle-locked"],
    [path.join(registryPath, "lock"), "registry-locked"],
  ]) {
    try {
      fs.lstatSync(file);
      block(code, "coordination");
    } catch (e) {
      if (e.code !== "ENOENT") {
        block(code, "coordination");
      }
  ...
  try {
    ...
    for (const dir of [backend, root, path.join(root, "provider")]) {
      ...
      if (inspect(file, true, false).size > 4096) {
        throw Error();
      }
      if (fs.readFileSync(file, "utf8") !== marker) {
        throw Error();
      }
    }
    const budget = { remaining: 10000 };
    contents(backend, budget);
    contents(root, budget);
  } catch {
    block("unsafe-state", "filesystem");
  }
  // One total budget for asynchronous evidence; adapters must remain read-only.
  const deadline =
    Date.now() +
    (Number.isSafeInteger(timeoutMs) && timeoutMs > 0 ? timeoutMs : 2000);
  ...
  for (const service of names) {
    ...
    try {
      ...
      block("process-not-stopped", service);
    }
    try {
      if (typeof portAvailable !== "function") {
        throw Error();
      }
      const available = await observe(() =>
        portAvailable(record.ports[service]),
      );
      if (available === false) {
        block("port-not-free", service);
      } else if (available !== true) {
        block("ports-unknown", service);
      }
    } catch {
      block("ports-unknown", service);
    }
