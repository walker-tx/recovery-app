...
async function fixture(t, fail = false, change = false) {
  ...
  const registry = {
    ...
    readOwned: async () => structuredClone(record),
  };
  const lifecycle = createLifecycle({
    registry,
    ready: async () => true,
    identify: async (name) =>
      structuredClone(record.processes[name.split("-").at(-1)]),
    run: async (_cmd, args) => {
      assert.ok(
        (
          await fs.stat(path.join(worktree, ".recovery-stack-lifecycle.lock"))
        ).isDirectory(),
      );
      const service = args[1].split("-").at(-1);
      calls.push(service);
      if (fail && service === "convexCloud") {
        throw Error("secret-canary");
      }
      states[service] = "stopped";
      if (service === "convexCloud") {
        states.convexSite = "stopped";
      }
      if (service === "mailpitHttp") {
        states.mailpitSmtp = "stopped";
      }
      if (change) {
        record.providerGeneration = randomUUID();
      }
    },
  });
...
test("partial stop reports completed, uncertain and unattempted process domains through CLI", async (t) => {
  const f = await fixture(t, true),
    output = [];
  const code = await runCli(["stop", f.record.stackId], {
    open: async () => ({
      stop: (id) => f.lifecycle.stop(f.worktree, id),
      close: async () => {},
    }),
  ...
  assert.deepEqual(report.processDomains, {
    metro: "stopped",
    convex: "uncertain",
  ...
  assert.ok(
    (
      await fs.stat(path.join(f.worktree, ".recovery-stack-lifecycle.lock"))
    ).isDirectory(),
  );
...
test("changed authoritative generation halts the already-started stop sequence", async (t) => {
  ...
  await assert.rejects(
    f.lifecycle.stop(f.worktree, f.record.stackId),
    /ownership/,
  );
...
test("owned-record inspection never recreates a missing reservation", async (t) => {
  ...
  await assert.rejects(
    registry.readOwned(worktree, randomUUID()),
    /ownership missing/,
  );
  await assert.rejects(fs.readFile(path.join(registryPath, "registry.json")), {
    code: "ENOENT",
  });
