...
async function fixture(t) {
  const worktree = await fs.realpath(
    await fs.mkdtemp(path.join(os.tmpdir(), "provider-destruction-")),
  );
  t.after(() => fs.rm(worktree, { recursive: true, force: true }));
  const registry = createRegistry({
    registryPath: path.join(worktree, "registry"),
  ...
  for (const dir of [root, provider]) {
    await fs.writeFile(path.join(dir, ".recovery-stack-owner.json"), marker, {
      mode: 0o600,
    });
  }
  for (const file of ["state.sqlite", "state.sqlite-wal", "state.sqlite-shm"]) {
    await fs.writeFile(path.join(provider, file), "synthetic", { mode: 0o600 });
  }
  await fs.writeFile(path.join(root, "mailpit.sqlite"), "preserve-inbox", {
    mode: 0o600,
  });
  let calls = 0;
...
test("confirmed stopped-provider destruction retires startup and preserves other domains and reservation", async (t) => {
  ...
  assert.deepEqual(
    await f.registry.readOwned(f.worktree, f.record.stackId),
    f.record,
  );
  await assert.rejects(
    f.lifecycle.start(f.worktree, () => {
      throw Error("definitions must not run");
    }),
    /retired/,
  );
  assert.equal(f.calls(), 0);
  await assert.rejects(
    f.lifecycle.destroyProvider(f.worktree, f.confirmation),
    /retired/,
  );
});
for (const scenario of [
  ...
  test(`${scenario} blocks without retirement or deletion`, async (t) => {
    ...
    if (scenario === "confirmation") {
      f.confirmation.affectedDomains = ["provider-data"];
    }
    if (scenario === "symlink") {
      await fs.rename(
        path.join(f.provider, "state.sqlite"),
        path.join(f.root, "saved"),
      );
      await fs.symlink("../saved", path.join(f.provider, "state.sqlite"));
    }
    ...
    if (scenario === "unknown-file") {
      await fs.writeFile(path.join(f.provider, "unowned"), "keep");
    }
    if (scenario === "foreign-marker") {
      await fs.writeFile(
        path.join(f.provider, ".recovery-stack-owner.json"),
        "{}",
      );
    }
    await assert.rejects(
...
test("running provider is refused before retirement", async (t) => {
  const f = await fixture(t),
    original = f.registry.status;
  f.registry.status = async (...args) => ({
    ...(await original(...args)),
    services: { provider: "running" },
  });
  await assert.rejects(
    f.lifecycle.destroyProvider(f.worktree, f.confirmation),
    /stopped/,
...
test("later unlink failure reports partial outcome and retains retirement and lifecycle exclusion", async (t) => {
  ...
  assert.deepEqual(result.storage, {
    "state.sqlite-wal": "removed",
    "state.sqlite-shm": "uncertain",
    "state.sqlite": "not-attempted",
  });
...
test("original file replacement after durable intent is not deleted", async (t) => {
  const f = await fixture(t),
    readOwned = f.registry.readOwned;
  let observations = 0;
  f.registry.readOwned = async (...args) => {
    ...
    if (++observations === 4) {
      ...
      await fs.writeFile(path.join(f.provider, "state.sqlite"), "replacement", {
        mode: 0o600,
      });
    }
    return record;
  ...
  assert.equal(
    await fs.readFile(path.join(f.provider, "state.sqlite"), "utf8"),
    "replacement",
  );
});
  ...
  test(`${failure} failure prevents every unlink and retains exclusion`, async (t) => {
    ...
    t.mock.method(fs, "open", async (file, ...args) => {
      ...
      if (file === path.join(f.root, "provider-retirement.json")) {
        return {
          // Combined failure verifies cleanup does not mask the write failure.
          writeFile: ["write", "write-and-close"].includes(failure)
            ? async () => {
                failedWrites++;
                throw Error("failure");
              }
          ...
          close: ["close", "write-and-close"].includes(failure)
            ? async () => {
                await handle.close();
                failedCloses++;
                throw Error("secret-close-canary");
              }
            : handle.close.bind(handle),
        };
      }
      if (file === f.root && failure === "directory-sync") {
        return {
          sync: async () => {
            throw Error("failure");
          },
    ...
    const result = await f.lifecycle.destroyProvider(
      f.worktree,
      f.confirmation,
    );
    assert.equal(result.state, "incomplete");
    ...
    assert.equal(
      failedWrites,
      ["write", "write-and-close"].includes(failure) ? 1 : 0,
    );
    assert.equal(
      failedCloses,
      failure === "close" ? 2 : failure === "write-and-close" ? 1 : 0,
    );
