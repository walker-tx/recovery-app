...
These are intended command contracts, not claims about current implementation. An explicit start refuses an already-running instance without preparation or another launch; a stopped restart reuses stable configuration and persisted identity. Do not routinely recreate databases, reseed users, or reinstall unchanged dependencies. Preparation failure is distinct from a ready stack. No detached agent task is treated as a durable service supervisor; persistent services belong to the existing process-management workflow.
...
Generate worktree-specific configuration through the repository's permitted Mise setup and preserve unrelated manual values. Do not wholesale copy a root checkout's secrets, Convex target, provider database, or signing identity. Separate nonsecret ownership bookkeeping from credential storage. Branch names are display labels, not authoritative identity keys. The approved local/automated-test simplification names a logical stack by its repository Git common-directory scope and canonical worktree directory basename, not its branch or historical directory inode. A name remains bound to one canonical path: a simultaneous or moved same-name path is refused rather than adopted automatically. Same-path directory recreation may select the same stopped logical stack; it is not proof that its old data survived. An established stack with missing or incompatible provider identity must refuse startup instead of generating new keys under the old provider generation. Process start identity, resource inode/ownership checks, port checks, and destructive-operation confirmation remain separate safety requirements. No nonce marker, automatic identity migration, or automatic reservation release is required or authorized. Previously host-wide same-name reservations block default repository-scoped selection until explicit ownership reconciliation; they are not silently copied or reset. Stale records, moved/deleted worktrees, and crashed processes require ownership checks before reclamation; PID alone is insufficient authority to kill a process.
