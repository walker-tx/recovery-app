...
- **Next action:** review
- **Last known-good commit:** This correction commit (`HEAD` after commit); section 3 still awaits fresh approval.
- **Correction cycles for current section:** 1
- **Last successful checks:** `mise exec -- pnpm --filter @recovery/mobile exec node --experimental-strip-types --test src/features/auth/auth-policy.test.ts src/features/auth/auth-submission.test.ts` passed 5/5; `mise exec -- pnpm --filter @recovery/backend exec node --experimental-strip-types --test convex/auth-policy.test.ts` passed 2/2; `mise exec -- pnpm --filter @recovery/mobile run check` passed; `mise exec -- pnpm --filter @recovery/backend run check` passed; and `mise run check` passed 2/2. Node emitted only the existing package-module-type warnings during focused tests.
- **Tests added in current section:** The correction changed the mobile policy regression to accept an existing eight-character credential while still rejecting an empty password, added backend profile-policy coverage that accepts and normalizes that credential without weakening unsupported-flow rejection, and connected submission retention coverage to the exact values passed from the screen state into the auth callback.
- **Review status:** Correction cycle 1 resolves both blocking findings from the review of `62bd24f` and now awaits a fresh review. The ten-character rule remains in `validatePasswordRequirements`, which pinned Auth invokes for signup/reset credential creation rather than returning-user sign-in.
- **Blocking condition:** None known pending fresh review. The package's direct sign-in errors/timing remain an inherited nonblocking limitation for this milestone because the documented protocol privacy gates defer signup/reset while the mobile returning-user flow sanitizes every displayed failure. Mixed-case legacy account population remains an open local-data compatibility question, not a verified finding.
