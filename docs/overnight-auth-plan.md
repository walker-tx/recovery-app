...
- [x] Add the smallest justified mobile tests for validation, safe error mapping, duplicate-submit prevention, and accessible labels/alerts if a compatible test harness can be introduced without broad infrastructure. Otherwise document bounded manual checks rather than adding speculative tooling.
...
Final verification completed with the local-anonymous Convex deployment as the confirmed backend target. Focused auth/profile policy, reducer, source-contract, and Convex harness tests passed, along with package and workspace checks, local function sync/codegen, and both native export bundles. Live device verification against the local backend remains manually unverified: the host had no configured simulator or physical device, so returning-user sign-in through profile onboarding and restoration, protected home, sign-out and protected-route removal, compact layouts, keyboard behavior, Dynamic Type, reading order, VoiceOver, and TalkBack were not exercised in a mounted app. Signup, verification, and reset remain intentionally unavailable under the reviewed pinned-package privacy/concurrency deferrals; no blocking condition prevents this milestone handoff.
