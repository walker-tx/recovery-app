...
const fixture = (
  ...
  Effect.gen(function* () {
    const dir = yield* Effect.acquireRelease(
      Effect.promise(() =>
        mkdtemp(join(tmpdir(), "verification-consumption-")),
      ),
      (ownedDir) =>
    ...
    const challenge = (email: string) =>
      Effect.promise(async () => {
        await sdk().userManagement.createUser({
          email,
        ...
        try {
          ...
          const verification = await sdk().userManagement.getEmailVerification(
            error.rawData.email_verification_id as string,
          );
...
it.live("verification is atomic single-use, user-bound and purpose-bound", () =>
  Effect.gen(function* () {
    ...
    const savedBody = String(
      f.db
        .prepare("SELECT body FROM email_verifications WHERE challenge_id=?")
...
it.live(
  ...
  () =>
    Effect.gen(function* () {
      ...
      assert.equal(
        (yield* Effect.promise(() => f.verify(a.pending, a.verification.code)))
          .user.emailVerified,
        true,
      );
    }),
