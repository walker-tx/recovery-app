...
import {
  ...
  WorkOSGatewayError,
} from "../../backend/convex/workosErrorPolicy.ts";
...
it.live(
  "actual SDK errors and configured client defaults satisfy existing gateway parser and categories",
  () =>
    Effect.gen(function* () {
      const dir = yield* Effect.acquireRelease(
        Effect.promise(() => mkdtemp(join(tmpdir(), "gateway-policy-"))),
        (ownedDir) =>
      ...
      yield* Effect.promise(() =>
        sdk.userManagement.createUser({ email, password }),
      );
      const rejects = (
      ...
      yield* rejects("authenticatePassword", "invalidCredentials", () =>
      ...
      yield* rejects("getEmailVerification", "invalidVerification", () =>
        sdk.userManagement.getEmailVerification("missing"),
      );
      yield* rejects("completeEmailVerification", "invalidVerification", () =>
      ...
      const verification = yield* Effect.promise(() =>
        sdk.userManagement.getEmailVerification(challenge.emailVerificationId),
      );
      ...
      yield* rejects("createPasswordReset", "invalidReset", () =>
        sdk.userManagement.createPasswordReset({
          email: "missing@example.test",
        }),
      );
      yield* rejects("completePasswordReset", "invalidReset", () =>
        sdk.userManagement.resetPassword({
          token: "missing",
          newPassword: password,
        }),
      );
      yield* rejects("refreshSession", "invalidSession", () =>
        sdk.userManagement.authenticateWithRefreshToken({
          refreshToken: "missing",
        }),
      );
      yield* rejects("revokeSession", "invalidSession", () =>
