    ...
    Effect.gen(function* () {
      const dir = yield* Effect.acquireRelease(
        Effect.promise(async () =>
          realpath(
            await mkdtemp(join(tmpdir(), "provider-destruction-integration-")),
          ),
        ),
        (ownedDir) =>
      ...
      yield* Effect.promise(async () => {
        ...
        for (const target of [root, providerDir]) {
          await writeFile(join(target, ".recovery-stack-owner.json"), marker, {
            mode: 0o600,
          });
        }
      });
      ...
      yield* Effect.promise(async () => {
        for (const file of preserved) {
          await writeFile(file, "owned-preserved-fixture", { mode: 0o600 });
        }
      });
      const snapshots = yield* Effect.promise(async () =>
        Promise.all(
          preserved.map(async (file) => ({
            file,
            bytes: await readFile(file),
            inode: (await lstat(file)).ino,
          })),
        ),
      );
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          const db = yield* Effect.acquireRelease(
            Effect.sync(() => new DatabaseSync(database, { readOnly: true })),
            (connection) => Effect.sync(() => connection.close()),
          );
          const saved = JSON.parse(
            String(
              db.prepare("SELECT body FROM instance WHERE id=1").get()?.body,
            ),
          );
          ...
          yield* Effect.promise(() =>
            assert.rejects(lstat(join(root, "provider-retirement.json")), {
              code: "ENOENT",
            }),
          );
        }),
      ...
      assert.deepEqual(yield* Effect.promise(() => readdir(providerDir)), [
      ...
      yield* Effect.promise(() =>
        assert.rejects(lstat(database), { code: "ENOENT" }),
      );
      assert.deepEqual(
        yield* Effect.promise(() =>
      ...
      for (const snapshot of snapshots) {
        assert.deepEqual(
          yield* Effect.promise(() => readFile(snapshot.file)),
          snapshot.bytes,
        );
        assert.equal(
          (yield* Effect.promise(() => lstat(snapshot.file))).ino,
          snapshot.inode,
        );
      }
      assert.equal(
      ...
      assert.deepEqual(
        yield* Effect.promise(() => registry.readOwned(sibling, other.stackId)),
        other,
      );
    }),
);
