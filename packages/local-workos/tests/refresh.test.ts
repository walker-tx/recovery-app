...
import { Effect, Layer, FileSystem, Path, Clock, Schema } from "effect";
...
const fixture = (sessionSeconds = 604800, accessTokenSeconds = 300) =>
  Effect.gen(function* () {
    ...
    const dir = yield* fs
      .makeTempDirectoryScoped({ prefix: "refresh-fixture-" })
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.sleep(
          Math.max(1, expiry - (yield* Clock.currentTimeMillis) + 1250),
        );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const original = yield* Effect.promise(f.signIn);
        const originalSid = yield* Schema.decodeUnknownEffect(Schema.String)(
          decodeJwt(original.accessToken).sid,
        );
        const sid = originalSid;
        const expiry = Number(
        ...
        yield* Effect.sleep(
          Math.max(1, expiry - (yield* Clock.currentTimeMillis) + 100),
        );
        yield* f.restart();
        for (const token of [original.refreshToken, rotated.refreshToken]) {
          yield* Effect.promise(() =>
            assert.rejects(f.refresh(token), invalid),
          );
        }
        assert.equal(
          f.db.prepare("SELECT COUNT(*) AS n FROM refresh_replays").get()?.n,
    ...
    () =>
      Effect.gen(function* () {
        ...
        f.db
          ...
          .run((yield* Clock.currentTimeMillis) - 1);
      ...
      Effect.gen(function* () {
        ...
        for (const action of ["reset", "revoke"]) {
          ...
          if (
            ...
            yield* Effect.promise(() =>
              assert.rejects(f.refresh(refresh.value.refreshToken), invalid),
            );
      ...
      Effect.gen(function* () {
        ...
        for (let i = 0; i < 256; i++) {
          current = yield* Effect.promise(() =>
            f.refresh(current.refreshToken),
          );
          if (i === 0) {
            first = current;
          }
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const unauthorized = yield* HttpClient.del(target);
        assert.equal(unauthorized.status, 401);
        const alias = yield* HttpClient.del(
          target.replace("/users/", "/%75sers/"),
        ...
        assert.equal(
          (yield* Effect.promise(() =>
            f.sdk().userManagement.getUser(other.id),
          )).id,
          other.id,
        );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        f.db
          .prepare("UPDATE instance SET body=? WHERE id=1")
          .run(savedIdentity);
