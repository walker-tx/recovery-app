...
import { acquireProvider, ProviderClearError } from "../src/provider.ts";
const fixture = Effect.gen(function* () {
  const dir = yield* Effect.acquireRelease(
    Effect.promise(async () =>
      realpath(await mkdtemp(join(tmpdir(), "provider-clear-"))),
    ),
    (ownedDir) =>
  ...
  () =>
    Effect.gen(function* () {
      ...
      const verified = yield* Effect.promise(() =>
        jwtVerify(session.accessToken, createLocalJWKSet(before), {
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(async () => {
        await rename(database, database + ".owned");
        await copyFile(database + ".owned", database);
      });
      const exit = yield* Effect.exit(f.provider.clearData(f.confirmation));
      assert.ok(Exit.isFailure(exit));
      assert.deepEqual(f.counts(), [1, 1, 1]);
