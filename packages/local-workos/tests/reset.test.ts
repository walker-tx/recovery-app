...
const fixture = (passwordResetSeconds = 1800) =>
  Effect.gen(function* () {
    const dir = yield* Effect.acquireRelease(
      Effect.promise(() => mkdtemp(join(tmpdir(), "reset-fixture-"))),
      (ownedDir) =>
  ...
  () =>
    Effect.gen(function* () {
      ...
      const saved = JSON.parse(
        String(
          f.db.prepare("SELECT body FROM users WHERE id=?").get(user.id)!.body,
        ),
      );
      f.db
        .prepare("UPDATE users SET body=? WHERE id=?")
        .run(JSON.stringify({ ...saved, email_verified: false }), user.id);
      let pending = "";
      ...
      assert.equal(
        (yield* Effect.promise(() => readFile(f.options.database))).includes(
          Buffer.from(reset.passwordResetToken),
        ),
        false,
      );
      yield* Effect.promise(() =>
...
it.live(
  ...
  () =>
    Effect.gen(function* () {
      ...
      f.db
        .prepare("UPDATE users SET body=? WHERE id=?")
        .run(JSON.stringify({ ...before, email_verified: false }), user.id);
      const reset = yield* Effect.promise(() =>
        f.sdk().userManagement.createPasswordReset({ email: user.email }),
      );
      yield* Effect.promise(() =>
