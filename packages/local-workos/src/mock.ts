...
import { InboxError, listInbox, readInbox } from "./admin-inbox.ts";
import { makeCommand } from "./mock-command.ts";
import type { Invocation } from "./mock-command.ts";
import {
  adminRequest,
  isMutation,
  selectTarget,
...
import type { Target } from "./mock-output.ts";
        ...
        invocation.operation === "version"
          ...
          : {
              ...
              safety: [
                ...
                "Pagination defaults to 50; pass returned cursor unchanged. Use the machine-safe wrapper mise exec -- ./scripts/mock.sh; mise run can add a runner banner on nonzero exits.",
              ],
              passwordExample:
                "printf '%s' 'synthetic-password' | mise exec -- ./scripts/mock.sh users create --email person@example.test --password-stdin --expect-stack <stack> --expect-generation <generation>",
            },
      ),
      json,
    );
