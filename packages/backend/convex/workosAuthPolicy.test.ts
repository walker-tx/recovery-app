...
describe("neutral WorkOS initiation policy", () => {
  it.each(initiationOutcomes)(
    "returns one opaque signup shape for every outcome",
    (outcome) => {
  ...
  it.each(initiationOutcomes)(
    "returns one neutral recovery shape for every outcome",
    (outcome) => {
  ...
  it("rejects non-opaque signup intent IDs", () => {
    expect(() =>
      signupInitiationResult({ kind: "new" }, "provider-user-id"),
    ).toThrow("Invalid signup intent");
  });
