...
it("reads the documented Convex runtime cloud URL rather than the CLI variable", () => {
  ...
  try {
    expect(workOSEnvironment().convexUrl === "http://127.0.0.1:6300").toBe(
      true,
    );
  } finally {
    vi.unstubAllEnvs();
...
describe("paired local trust", () => {
  ...
  it.each([
    ...
    expect(() =>
      buildWorkOSAuthConfig({ ...local, [key]: undefined }),
    ).toThrow();
  });
  it.each([
    { issuer: `${local.issuer}/` },
    ...
    { stackId: "not-a-uuid" },
    { stackId: "12345678-1234-1234-8234-123456789abc" },
    { stackId: "12345678-1234-4234-7234-123456789abc" },
    { apiUrl: "https://api.workos.com" },
    { jwks: "http://example.com/jwks" },
    { convexUrl: "https://example.convex.cloud" },
    { apiUrl: "http://127.0.0.1:6100/path" },
    { apiUrl: "http://secret@127.0.0.1:6100" },
    { deployKey: "inherited" },
  ...
  it.each([
    ...
    "jwks",
    "apiUrl",
  ] as const)("staging rejects %s override", (key) => {
    expect(() =>
      buildWorkOSAuthConfig({
        mode: "staging",
        workosClientId: "client_01ABC123",
...
describe("local credential and deployment isolation", () => {
  it("uses explicit local credentials, permitting a matching generic key", () => {
    expect(
      resolveWorkOSApiKey({ mode: "local", localApiKey: localKey }) ===
        localKey,
    ).toBe(true);
    expect(
      resolveWorkOSApiKey({
    ...
    expect(
      resolveWorkOSApiKey({ mode: "staging", apiKey: "synthetic-staging" }) ===
        "synthetic-staging",
    ).toBe(true);
  });
  it("rejects missing, generic-only, malformed, and conflicting credentials", () => {
    for (const localApiKey of [
      undefined,
      "",
      "sk_test_synthetic",
      `sk_test_local_${"a".repeat(63)}`,
      `sk_test_local_${"A".repeat(64)}`,
      `sk_test_local_${"a".repeat(65)}`,
    ]) {
      let rejected = false;
      try {
        resolveWorkOSApiKey({ mode: "local", localApiKey, apiKey: localKey });
      } catch {
        rejected = true;
      }
    ...
    try {
      resolveWorkOSApiKey({
        mode: "local",
        localApiKey: localKey,
        apiKey: "synthetic-real-test-key",
      });
    } catch {
      rejected = true;
    }
  ...
  it("rejects remote and malformed selectors", () => {
    for (const deployment of [
      ...
      let rejected = false;
      try {
        buildWorkOSAuthConfig({ ...local, deployment });
      } catch {
        rejected = true;
      }
      expect(rejected).toBe(true);
  ...
  it("allows absent or valid local selectors", () => {
    for (const deployment of [
      undefined,
      "",
      "local:synthetic_123-abc",
      "anonymous:synthetic-123",
    ]) {
      expect(
        buildWorkOSAuthConfig({ ...local, deployment }).providers.length === 1,
      ).toBe(true);
    }
  });
