...
export function resolveWorkOSExpectations(env: WorkOSAuthConfigEnvironment) {
  ...
  if (env.mode !== "local") {
    throw new Error("WORKOS_MODE must be staging or local");
  }
  if (
    !env.stackId ||
    !UUID.test(env.stackId) ||
    !env.providerGeneration ||
    !UUID.test(env.providerGeneration)
  ) {
    throw new Error(
      "Local auth requires paired stack and provider-generation UUIDs",
    );
  }
  ...
  if (
    ...
    env.issuer !== issuer ||
    env.audience !== expectedClient
  ) {
    throw new Error(
      "Local issuer, client and audience must match provider generation",
    );
  }
  if (
    env.deployment &&
    (env.deployment.trim() !== env.deployment ||
      !/^(local|anonymous):[A-Za-z0-9_-]+$/.test(env.deployment))
  ) {
    throw new Error("Local auth rejects nonlocal CONVEX_DEPLOYMENT");
  }
  if (env.deployKey) {
    throw new Error("Local auth rejects inherited CONVEX_DEPLOY_KEY");
  }
  ...
  return {
    ...
    sdkOptions: {
      ...
      port: Number(api.port || (api.protocol === "https:" ? 443 : 80)),
    },
  };
}

export function buildWorkOSAuthConfig(
  env: WorkOSAuthConfigEnvironment,
): AuthConfig {
    ...
    providers: [
      {
        ...
        jwks: trust.jwks,
        algorithm: "RS256",
        ...(trust.audience === undefined
          ? {}
          : { applicationID: trust.audience }),
      },
    ],
  };
}
...
export function resolveWorkOSApiKey(env: WorkOSAuthConfigEnvironment): string {
  if (env.mode === "local") {
    ...
    if (
      !key ||
      key.trim() !== key ||
      !/^sk_test_local_[0-9a-f]{64}$/.test(key)
    ) {
  ...
  if (!env.apiKey) {
    throw new Error(
      "Missing required server environment variable: WORKOS_API_KEY",
    );
  }
  return env.apiKey;
}
