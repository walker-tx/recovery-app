...
function point(ns,box){const viewport=ns.map(bounds).filter(Boolean).sort((a,b)=>(b[2]*b[3])-(a[2]*a[3]))[0];if(!viewport||viewport[0]!==0||viewport[1]!==0||box[2]>viewport[2]||box[3]>viewport[3])fail();const percentages=[0,1].map(i=>Math.round((box[i]+box[i+2])/2/viewport[i+2]*100));for(const i of [0,1]){const target=Math.floor(viewport[i+2]*percentages[i]/100);if(!Number.isFinite(target)||percentages[i]<0||percentages[i]>100||target<=box[i]||target>=box[i+2])fail();}return `${percentages[0]}%,${percentages[1]}%`;}
