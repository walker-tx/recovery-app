import type { Doc } from '@recovery/backend/convex/_generated/dataModel';
...
export function CountReading({ count, now, size = 'row' }: { count: Doc<'counts'>; now: number; size?: 'row' | 'detail' }) {
...
export function CountRow({ count, now, onPress }: { count: Doc<'counts'>; now: number; onPress: () => void }) {
