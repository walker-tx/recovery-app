...
test("auth failures map to fixed purpose-specific messages without leaking provider details", () => {
  ...
  for (const purpose of ["signup", "verification", "recovery", "reset"] as const) {
    const message = toSafeAuthError(purpose, raw);
    assert.doesNotMatch(message, /WorkOS|secret-token|known@example\.com/);
  }
  assert.equal(toSafeAuthError("verification", raw), "That code is invalid or expired. Start signup again if you need a new code.");
  assert.equal(toSafeAuthError("recovery", raw), "We couldn't start password recovery. Try again.");
});
