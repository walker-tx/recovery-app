...
test("signup intent lifecycle is in-memory and clears on completion or back-to-welcome", () => {
  ...
  assert.deepEqual(signupFlowReducer(started, { type: "completed" }), {
    intentId: null,
    submittedEmail: null,
  });
  assert.deepEqual(signupFlowReducer(started, { type: "backToWelcome" }), {
    intentId: null,
    submittedEmail: null,
  });
});
...
test("signup provider has no persistence or credential ownership", async () => {
  const source = await readFile(
    new URL("./signup-flow-provider.tsx", import.meta.url),
    "utf8",
  );
  assert.doesNotMatch(
    source,
    /SecureStore|AsyncStorage|accessToken|refreshToken/,
  );
});
