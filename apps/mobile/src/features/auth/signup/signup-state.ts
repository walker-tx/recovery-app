...
export function getSignupCooldownSecondsRemaining(
  state: SignupState,
  now: number,
) {
  if (state.cooldownEmail !== normalizeEmail(state.email)) {
    return 0;
  }
...
export type SignupAction =
  ...
  | { type: "submissionAccepted"; acceptedAt: number; submittedEmail: string }
  | { type: "submissionFailed"; message: string };
export function reduceSignupState(
  state: SignupState,
  action: SignupAction,
): SignupState {
  switch (action.type) {
    ...
    case "submissionAccepted":
      return {
        ...state,
        isPending: false,
        cooldownEmail: normalizeEmail(action.submittedEmail),
...
export function reduceVerificationState(
  ...
  switch (action.type) {
    case "codeChanged":
      return { ...state, code: action.value, formError: null };
    case "submissionStarted":
      return { ...state, isPending: true, formError: null };
