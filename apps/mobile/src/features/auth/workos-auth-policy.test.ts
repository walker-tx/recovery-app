...
test("validates WorkOS sign-in fields and chooses the first invalid field", () => {
  ...
  assert.equal(
    getFirstInvalidWorkOSSignInField(getWorkOSSignInValidation("person@example.com", "")),
    "password",
  );
  assert.deepEqual(getWorkOSSignInValidation(" Person@Example.COM ", "password"), {});
});
...
test("maps every WorkOS credential failure to one neutral message", () => {
  ...
  assert.equal(safe, "We couldn't sign you in. Check your email and password, then try again.");
  assert.equal(safe.includes(providerDetail), false);
});

test("unauthenticated startup resolves only to auth after restoration", () => {
  assert.equal(
    getWorkOSRouteDestination({ isLoading: true, isAuthenticated: false }, undefined),
    "loading",
  );
  assert.equal(
...
test("authenticated routing waits for profile resolution without flashing a route", () => {
  assert.equal(
    getWorkOSRouteDestination({ isLoading: false, isAuthenticated: true }, undefined),
    "loading",
  );
  assert.equal(
    getWorkOSRouteDestination({ isLoading: false, isAuthenticated: true }, null),
    "onboarding",
  );
  assert.equal(
    getWorkOSRouteDestination(
