...
export function getResetValidation(
  token: string,
  password: string,
  confirmation: string,
): ResetValidation {
  ...
  if (token.trim().length === 0) {
    errors.token = "Enter the reset token from your recovery email.";
  }
...
export type RecoveryAction =
  ...
  | { type: "submissionSucceeded"; acceptedAt: number; submittedEmail: string }
...
export function reduceRecoveryState(
  ...
  switch (action.type) {
    ...
    case "submissionStarted":
      return { ...state, isPending: true, formError: null };
    case "submissionSucceeded":
      return {
        ...state,
        submittedEmail: action.submittedEmail,
        isPending: false,
        cooldownUntil: action.acceptedAt + RESEND_COOLDOWN_MS,
      };
    case "submissionFailed":
      return { ...state, isPending: false, formError: action.message };
  }
}
...
export type ResetAction =
  ...
  | { type: "submissionFailed"; message: string };
export function reduceResetState(
  state: ResetState,
  action: ResetAction,
): ResetState {
  switch (action.type) {
    case "tokenChanged":
      return { ...state, token: action.value, formError: null };
    case "passwordChanged":
      return { ...state, password: action.value, formError: null };
    case "confirmationChanged":
      return { ...state, confirmation: action.value, formError: null };
    case "submissionStarted":
      return { ...state, isPending: true, formError: null };
    case "submissionSucceeded":
