...
const variants: Record<Variant, { tag: string; text: string }> = {
  accentTinted: {
    tag: "border-blueprint-tint bg-blueprint-tint",
    text: "text-blueprint-pressed",
  },
  accentOutlined: {
    tag: "border-blueprint bg-transparent",
    text: "text-blueprint-pressed",
  },
...
export function Tag({
  ...
  return (
    <View
      className={`self-start rounded-subtle border px-sm py-xs ${palette.tag} ${className ?? ""}`}
      {...props}
    >
      <Typography
        className={`font-bold ${palette.text}`}
        variant="caption"
      >
        {children}
      </Typography>
